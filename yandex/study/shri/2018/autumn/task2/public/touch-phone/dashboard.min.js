...
modules.define('scroller', ['i-bem-dom', 'events'/*, 'events__channels'*/], function(provide, bemDom, events, channels) {
    ...
    onSetMod: {
        js: {
            inited: function() {
                ...
                if ($(this.domElem[0]).hasClass('favorite-devices__scroller')) {
                    parent = '.favorite-devices';
                } else if ($(this.domElem[0]).hasClass('favorite-scenarios__scroller')) {
                    parent = '.favorite-scenarios';
                } else {
                    return false;
                }

                // console.log(that, $(this.domElem[0]).parents('favorite-scenarios'));

                pager_right = $(this.domElem[0]).parents(parent).eq(0).find('.icon__arrow_position_right');
                pager_left = $(this.domElem[0]).parents(parent).eq(0).find('.icon__arrow_position_left');
                

                if (!pager_right.length || !pager_left.length) {
                    return false;
                }
                // console.log($(pager_right), $(pager_left));



                var visible_founded;
                $(pager_right).on('click', function() {
                    visible_founded = false;
                    if (!$(pager_right).hasClass('icon__arrow_inactive')) {
                        var childs = that.findChildElems('scroll-item');
                        for (var i=0; i<childs._entities.length; i++) {
                            // console.log(i, visible_founded);
                            // check only available for scrolling, skip hidden
                            // if (childs._entities[i].hasMod('invisible')) {
                            if (!$(childs._entities[i].domElem[0]).hasClass('device_invisible')) {
                                if (!checkVis(childs._entities[i].domElem[0], 50, 50 )) {
                                    if (visible_founded) {
                                        // console.log('scroll to ', childs._entities[i].domElem[0]);
                                        return  scrollDiv(that.domElem[0], childs._entities[i].domElem[0], 'left');
                                    }
                                } else {
                                    visible_founded = true;
                                }
                            }
                        }
                ...
                $(pager_left).on('click', function() {
                    visible_founded = false;
                    if (!$(pager_left).hasClass('icon__arrow_inactive')) {
                        var childs = that.findChildElems('scroll-item');
                        for (var i=childs._entities.length-1; i>=0; i--) {
                            ...
                            if (!$(childs._entities[i].domElem[0]).hasClass('device_invisible')) {
                                if (!checkVis(childs._entities[i].domElem[0], 50, 50 )) {
                                    if (visible_founded) {
                                        // console.log('scroll to ', childs._entities[i].domElem[0]);
                                        return  scrollDiv(that.domElem[0], childs._entities[i].domElem[0], 'right');
                                    }
                                } else {
                                    visible_founded = true;
                                }
                ...
                // console.log(that, pager_right);
                

                // var child = this.findChildElem(Modal);
                this._domEvents().on('scroll', function() {

                	var childs = that.findChildElems('scroll-item');
                    var first_visible_child, last_visible_child;
                    var visible_childs=[];

                    // first_visible_child = childs._entities[0].domElem[0];
                    // last_visible_child = childs._entities[childs._entities.length-1].domElem[0];

                    // first_visible_child = childs._entities.find(fucntion(el){
                    //     return !$(el.domElem[0]).hasClass('device_invisible')
                    // });
                    
                    visible_childs = childs._entities.filter(function(el){return !$(el.domElem[0]).hasClass('device_invisible')});
                    first_visible_child = visible_childs[0].domElem[0];
                    last_visible_child = visible_childs[visible_childs.length-1].domElem[0];

                    // check if the last child visible
                    if (visible_childs.length) {
                        // console.log(childs._entities[childs._entities.length-1]);

                        // костыль..
                        // привет jquery
                        // FIX IT

                        // add styles for right arrow
                        if (checkVis(last_visible_child, 50, 50)) {
                            // pager_right =  that.findChildElems('arrow-right');
                            // console.log(pager_right);
                        

                            
                            $(pager_right).removeClass('icon__arrow_active').addClass('icon__arrow_inactive');
                        } else {
                            $(pager_right).addClass('icon__arrow_active').removeClass('icon__arrow_inactive');
                        }

                        // костыль..
                        // привет jquery
                        // FIX IT
                        if (checkVis(first_visible_child, 50, 50)) { 
                                
                            // visible
                            // console.log('visible',childs._entities[0].domElem);
                            $(pager_left).removeClass('icon__arrow_active').addClass('icon__arrow_inactive');
                        } else {
                            // hidden
                            // console.log('hidden',childs._entities[0].domElem);
                            $(pager_left).addClass('icon__arrow_active').removeClass('icon__arrow_inactive');
                        }

                        // add styles for left arrow
                    ...
                    // console.log(that);
                    // check if scroll container is visible and diable all arrows
                    if ($(that.domElem[0]).find('.favorite-devices__devices')[0] && checkVis($(that.domElem[0]).find('.favorite-devices__devices')[0])) {
                        // console.log('hide arrows', pager_left, pager_right);
                        $(pager_left).removeClass('icon__arrow_active').addClass('icon__arrow_inactive');
                        $(pager_right).removeClass('icon__arrow_active').addClass('icon__arrow_inactive');
                    }

                    // console.log('scrolled', childs, that);
                    // $.each(childs._entities, function(i, el){
                    //     // console.log(el);
                    //     // console.log();
                    //     // if (!isInViewport(el.domElem[0], $(that.domElem[0]))) {
                    //     if (!checkVis(el.domElem[0])) {
                    //         console.log('hidden', el.domElem[0]);
                    //     }
                    // })
                	// myEmitter.emit('myevent');
                	// myChannel.emit('test', { foo : 'bar' });
                	// var myEvent = new events.Event('myevent');
            		// myEmitter.emit(myEvent);
                })
                ...
                function checkVis(element, percentX, percentY){
                ...
                function scrollDiv(container, scrollToElem, direction, duration) {
                    if (!duration)
                        duration = 1000;
                    ...
                    if (direction == 'left') {
                        leftPosition = $(scrollTo).width() - $(container).width() + 38;
                        // leftPosition  = $(scrollTo).width() - $(container).width() + $(scrollTo).parent().children().eq(0).offset().left -$(scrollTo).parent().offset().left;
                        // leftPosition = $(scrollTo).parent().offset().left - $(scrollTo).parent().children().offset().left;
                    } else if (direction == 'right') {
                        leftPosition = $(scrollTo).parent().offset().left - $(scrollTo).parent().children().eq(0).offset().left -3;
                    }
                    // -($(container).offset().left - $(scrollTo).offset().left);

                    // leftPosition = -1000;

                    // console.log(container, scrollTo);
                    // console.log(container.offset().left, container.scrollLeft(), scrollTo.offset().left, $(scrollTo).width(), $(window).width(), leftPosition);
                    
                    // console.log(scrollTo.offset().left - container.offset().left + container.scrollLeft()  + leftPosition);

                    container.animate({
                        scrollLeft: scrollTo.offset().left - container.offset().left + container.scrollLeft()  + leftPosition
                    }, duration);
                } 
            }
        }
...
/* begin: ../../common.blocks/device/device.js */
/*modules.define('device', ['BEMHTML', 'i-bem-dom'], function(provide, BEMHTML, bemDom) {
	...
	provide(bemDom.declBlock(this.name, {
	    ...
	    _onCLick: function () {
	    	...
	    	// console.log(this.findChildBlock(Icon));
	    }
	}));
...
modules.define('device', ['BEMHTML', 'i-bem-dom', 'events'/*, 'events__channels'*/], function(provide, BEMHTML, bemDom, events, channels) {
	    ...
	    onSetMod : {
	        js : {
	            inited : function() {
	                ...
	                // var modal = this.findChildElem('modal');

	                this._domEvents().on('click', function(ev) {
	                 	// console.log('clicked', this, ev);
	                 	// ev.preventDefault();
	                	// console.log(modal);
	                    // modal.setMod('become-bigger', true);
	                    // modal.setMod('visible', true);
	                    // modal.setMod('animated', true);

	                    // костыль..
				    	// привет jquery
				    	// FIX IT
	                    // $('.page__inner').addClass('page__inner-blured');

	                    // modal.setMod('ios-animated', true);
...
/* end: ../../common.blocks/device/device.js */
/* begin: ../../common.blocks/ios-modal/ios-modal.js */
modules.define('ios-modal', ['i-bem-dom'], function(provide, bemDom) {
    ...
    onSetMod: {
        js: {
            inited: function() {
            	var that = this;
                // console.log(that);
                var parent = $(that.domElem[0]).parent();
                $(parent).on('click', function(){
                	// console.log('open modal', that);
                	$('.page').append(that.domElem[0]);
                	that.setMod('visible');


                	var content = that.findChildElem('content');
                	var parent_width = $(parent).width();
                	var parent_height = $(parent).height();
                	var content_width = $(content.domElem[0]).width();
                	var content_height = $(content.domElem[0]).height();
                	// console.log('parent', parent_width, $(parent).offset().left, $(parent).offset().top);
                	// console.log(content_width);
                	// var scale = parent_width / content_width ;
                	var scale;
                	if (content_height>content_width) {
                		scale = parent_height / content_height ;
                	} else {
                		scale = parent_width / content_width ;
                	}
                	// console.log(scale);


                	// change initial size of element == parent by scaling
                	// and place at the same position as parent
                	function changeSizeAndPlace() {

	                	$(content.domElem[0]).animate({ 
	                		
	                		left:  $(parent).offset().left,
	                		top: $(parent).offset().top,
	                		left:  $(parent).offset().left - (1-scale)*content_width/2,
	                		top: $(parent).offset().top - (1-scale)* content_height/2 + (parent_height-content_height*scale)/2,
	                		// width: "70%",
	                		// transform: 'scale(0.2)'
	                	}, {
						    step: function(now,fx) {
						    	if (fx.prop=='left') {
						    		var cur_change = (fx.now - fx.start)/(fx.end-fx.start);
						    		cur_scale = cur_change* (scale-1) +1;
						    		// console.log(cur_scale);
						      		$(this).css('-webkit-transform','scale('+cur_scale+')'); 
						      		$(this).css('-ms-transform','scale('+cur_scale+')'); 
						      		$(this).css('transform','scale('+cur_scale+')'); 
						      		// console.log(now, fx.now, fx.start, fx.end);
						    	}
						      	
						    },
						    duration: 0,
						    complete: startAnimationModalAppear,
						},'linear', );
                	
                	}
                	changeSizeAndPlace();

                	// calculate target coordinates and start animation
                	function startAnimationModalAppear() {
                		blurPage();
                		// console.log('startAnimationAppear');
                		// console.log($(content.domElem[0]));
                		$(content.domElem[0]).animate({ 
	                		
	                		left:  ($(window).width() - content_width) /2,
	                		top: ($(window).height() - content_height) /2,
	                		// width: "70%",
	                		// transform: 'scale(0.2)'
	                	}, {
						    step: function(now,fx) {
						    	if (fx.prop=='left') {
						    		var cur_scale = (fx.now - fx.start)/(fx.end-fx.start);
						    		// console.log(cur_scale);
						      		$(this).css('-webkit-transform','scale('+cur_scale+')'); 
						      		$(this).css('-ms-transform','scale('+cur_scale+')'); 
						      		$(this).css('transform','scale('+cur_scale+')'); 
						    ...
						    duration: 500,
						    complete: finishModalAppear,
						},'linear', );
                	}

                	function blurPage() {
                		// console.log('blurPage');
                		// ios-modal_background-animated
                		that.setMod('background-animated');
                	}

                	function finishModalAppear() {
                		$('.page__inner').addClass('page__inner_blur-animated');

                		$(content.domElem[0]).css('position','static');

                		setTimeout(function(){
                			// startAnimationModalDisappear();
                		}, 3000);
                	}

                	function startAnimationModalDisappear() {
                		$('.page__inner').removeClass('page__inner_blur-animated');
                		that.delMod('background-animated');

                		// console.log('startAnimationModalDisappear');
                		// console.log($(content.domElem[0]));
                		$(content.domElem[0]).css('position','absolute');
                		$(content.domElem[0]).animate({ 
                	...
                	$(content.domElem[0]).find('.ios-modal__button-ok,.ios-modal__button-cancel').off('click').on('click', function(){
						startAnimationModalDisappear();
                	})

                		/*if ($(ev.target).is('.ios-modal__button-ok') || $(ev.target).is('.ios-modal__button-cancel')) {
                			startAnimationModalDisappear();
                		}*/
...
/* end: ../../common.blocks/ios-modal/ios-modal.js */
/* begin: ../../common.blocks/device-settings/device-settings.js */
modules.define('device-settings', ['i-bem-dom'], function(provide, bemDom) {
        ...
        js: {
            inited: function() {
                ...
                this._domEvents().on('click', function(ev){
	            	...
	            	// привет jquery
	            	// FIX IT
                	if ($(ev.target).is('.badges__badge')) {
                		var data = $(ev.target).data('value');
                        // console.log(data);
                        // data = JSON.parse(data);
                		$(ev.target).parents('.device-settings').eq(0).find('.horizontal-slider__input-range').val(data.val).change();

                        $(ev.target).removeClass('badges__badge_selected_no').removeClass('badges__badge_selected_yes');
                		$(ev.target).siblings().addClass('badges__badge_selected_no');
                		$(ev.target).addClass('badges__badge_selected_yes');

                	}
                	if ($(ev.target).is('.device-settings__button-ok') || $(ev.target).is('.device-settings__button-cancel')) {
                		
                		// var modal = this.findParentBlocks(Modal);
				    	...
				    	// привет jquery
				    	// FIX IT
	                    // $('.page__inner').removeClass('page__inner-blured');
                		// modal.setMod('visible', false);
                	}
                });
            }
...
/* end: ../../common.blocks/device-settings/device-settings.js */
/* begin: ../../common.blocks/horizontal-slider/horizontal-slider.js */
modules.define('horizontal-slider', ['i-bem-dom'], function(provide, bemDom) {
    ...
    onSetMod: {
        js: {
            inited: function() {
                ...
                // this._domEvents().on('mousemove', this._updateCurValue);
                this._domEvents().on('input change', this._updateCurValue);
                this._domEvents().on('change', this._updateCurValue);
                this._domEvents().on('drag', console.log);
            }
        }
    },
    _updateCurValue: function() {
    	// костыль..
    	// привет jquery
    	// FIX IT
    	var val = $(this.domElem[0]).find('input').val();
    	val = val>0 ? ('+'+val) : val; 
        // console.log( $(this.domElem[0]));
    	$(this.domElem[0]).parents('.device-settings').eq(0).find('.device-settings__cur-mode').text(val);
        $(this.domElem[0]).parents('.device-settings').eq(0).find('.badges__badge').removeClass('badges__badge_selected_yes');
        $(this.domElem[0]).parents('.device-settings').eq(0).find('.badges__badge').eq(0).removeClass('badges__badge_selected_no').addClass('badges__badge_selected_yes');
...
/* end: ../../common.blocks/horizontal-slider/horizontal-slider.js */
/* begin: ../../common.blocks/circle-slider/circle-slider.js */
modules.define('circle-slider', ['i-bem-dom'], function(provide, bemDom) {
...
provide(bemDom.declBlock(this.name, {
    onSetMod: {
        js: {
            inited: function() {
            	var that = this;
                /*$(".circle-slider__handle").roundSlider({
				    radius: 110.5,
				    value: 25,
				...
				var data = $(that.domElem[0]).data('value');

				$(that.domElem[0]).parents('.device-settings').find(".circle-slider__handle").roundSlider({
				    radius: 111,
				    value: '+'+data.val,
				    mouseScrollAction: true,
				    animation: false,
				...
				function _updateInfo(args){
			    	// console.log(args);
			        var val = parseInt(args.value);
			        val = val>0 ? ('+'+val) : val; 
			        $(that.domElem[0]).parents('.device-settings').eq(0).find('.device-settings__cur-mode').text(val);
			        $(that.domElem[0]).parents('.device-settings').eq(0).find('.circle-slider__cur-mode-value').text(val);
			    }
				...
				// Modify lib
				$(that.domElem[0]).parents('.device-settings').eq(0).find('.rs-container').find('.rs-block.rs-outer').append('<div class="rs-overlay rs-transition rs-bg-color"></div>');

				/*
				background-image: url('/assets/dashboard/Oval@2x.png');
				background-size: 100%;

				background-image: url('/assets/fixed/Oval@2x_grey_full.png');
				background-size: 100%;

				background-image: url('/assets/fixed/icon_arrow.svg');
				*/
            }
        }
    }
...
/* end: ../../common.blocks/circle-slider/circle-slider.js */
/* begin: ../../touch-phone.blocks/favorite-devices/favorite-devices.js */
modules.define('favorite-devices', ['i-bem-dom'], function(provide, bemDom) {
    ...
    onSetMod: {
        js: {
            inited: function() {
				...
				var trackChange = function(element, property, cb) {
				  var observer = new MutationObserver(function(mutations, observer) {
				    if(mutations[0].attributeName == property) {
				...
				trackChange($(that.domElem[0]).find('.favorite-devices__filter').find('button')[0], "aria-activedescendant", function(){
					// console.log('changed');
					
					var cur_filter_id = $(that.domElem[0]).find('[name=filter-favorite-devices]').val();
					var devices;
					if (cur_filter_id) {
						devices = $(that.domElem[0]).find('.device');
						devices.map(function(id){
                			$(devices[id]).removeClass('device_invisible');
                			if ($(devices[id]).data('value').filter_ids.indexOf(cur_filter_id)==-1) {
                				$(devices[id]).addClass('device_invisible');
                			}
                		});
                		$(that.domElem[0]).find('.favorite-devices__scroller').animate({
							scrollLeft:0
						}, 100);
					}
				...
				replaceWithWrapper($(that.domElem[0]).find('.favorite-devices__filter').find('button')[0], "aria-labelledby", function(obj, property, value) { 
...
/* end: ../../touch-phone.blocks/favorite-devices/favorite-devices.js */
